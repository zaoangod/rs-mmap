...
它提供与 [memmap2](https://github.com/RazrFalcon/memmap2-rs) 相同风格的 `Mmap`、`MmapOptions` 与 `MmapAsRawDesc`，但刻意只保留读取文件所需的最小 interface。
...
fn main() -> std::io::Result<()> {
    ...
    // Mmap 不借用 File；创建成功后可关闭或丢弃 file。
    drop(file);
...
除 `&File` 外，`map` 还接受实现 `MmapAsRawDesc` 的来源：Unix 上为 `RawFd` 或
